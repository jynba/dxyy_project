...
Page({
  ...
  hotSelect: async function (e) {
    ...
    // if (!order) {
      that.getData(res => {
        that.concatData(that.data.community)
      }, that.data.orderBy);
    // }
    ...
    // }
    //     that.setData({
    //       community: [...that.data.community, ...res.result.data],
    //       allcommunity: [...that.data.allcommunity, ...res.result.data],
    //       isEndOfList: true
    //     }, res => {
    //       that.concatData(that.data.community);
    //       wx.hideLoading()
  ...
  getData: function (callback, orderby) {
    ...
    var that = this;
    var openid = wx.getStorageSync('clientId')
    mycommunity.orderBy(orderby, 'desc') //按创建时间的最新开始排列(降序)
...
//数据传到本地
        that.setData({
          allcommunity: [...that.data.allcommunity, ...res.data],
          community: [...that.data.community, ...res.data],
  ...
  onLoad: function (options) {
    ...
    // var community = this.data.community.slice(0) //必须深拷贝
    this.getData(res => {
      this.concatData(this.data.community)
    }, this.data.orderBy);
  ...
  submit: function (e) {
    ...
    if (!avatarUrl || !username) {
      ...
      wx.navigateTo({
        url: '/subPackages/pages/edit/edit',
      })
    }
  },
  islike: function (e) {
