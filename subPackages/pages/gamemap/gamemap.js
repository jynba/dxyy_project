...
Page({
    ...
    onLoad: function (option) {
        let that = this;
        const gamename = wx.getStorageSync('gamename')
        ...
        if (option.name) {
            ...
            wx.setStorageSync('gamename', option.name)
        } else if (gamename) {
            that.setData({
                name: gamename,
                avatarUrl: avatarUrl,
        ...
        if (clientId) {
            activity.doc(option.id).get().then(res => {
                ...
                let isTeamWork = that.data.isTeamWork;
                ...
                if (res.data.itemRaceSystem === "团体赛") {
                    isTeamWork = true;
                }
                that.setData({
                    mode,
        ...
        that.onlyOneFn = once(function () {
            ...
            activity.doc(option.id).get().then(res => {
                ...
                switch (active) {
                    ...
                    case 2:
                        mode = '猜地点模式';
                        hiddenMarkers = markers.splice(0,markersLen);
                        console.log(hiddenMarkers);
                        nowMarkersLen = 0;
                        //清空markers中的定义的标记点，赋值到hiddenMarkers
    ...
    getbackgroudloation: function () {
        ...
        var o = function (e) {
            ...
            if (y - I <= 2)
                ...
                d.teamId = that.data.teamId;
                d.polyline = that.data.polyline;
                d.endState = that.data.endState;
                d.totallen = that.data.totallen;
                d.costTime = that.data.costTime;
                d.reachPoints = that.data.reachPoints;
                var r = new Date();
                d.time = r.toString();
                        ...
                        case 'refresh':
                            ...
                            wx.getLocation({
                                ...
                                success: (res) => {
                                    ...
                                    const totallen = that.data.totallen;
                                    const costTime = that.data.costTime;
                                    const reachPoints = that.data.reachPoints;
                                    ...
                                    UpdateObj.teamId = teamId;
                                    UpdateObj.reachPoints = reachPoints;
                                    UpdateObj.polyline = polyline;
                                    UpdateObj.endState = endState;
                                    UpdateObj.totallen = totallen;
                                    UpdateObj.costTime = costTime;
                                    let time = new Date()
                                    UpdateObj.time = time;
    ...
    addmarker(latitude, longitude, obj, clientId, timestamp) {
        ...
        var that = this,
            ...
            allUserMarkers = that.data.allUserMarkers,
            isSame = false;
        let avatarUrl = obj.avatarUrl;
        let costTime = parseInt(obj.costTime) ? parseInt(obj.costTime) : 0;
        let totallen = parseInt(obj.totallen) ? parseInt(obj.totallen) : 0;
        let username = obj.name;
        let reachPoints = obj.reachPoints;
        let polyline = obj.polyline;
        let teamId = obj.teamId;


        for (let index = 0; index < allUserMarkers.length; index++) {
            const obj = allUserMarkers[index];
            ...
            if (obj.clientId === clientId) { //已存在的用户更新气泡数据
                ...
                obj.totallen = totallen;
                obj.reachPoints = reachPoints;
        ...
        // 当前为所有用户都有的实时数据
        // 判断赛制，区分团体赛和个人赛，不显示所有用户数据
        // 同一个队伍下的才能共享位置
        let addMarkers;
        if (that.data.isTeamWork) {
            addMarkers = allUserMarkers.filter(item =>
                item.teamId == that.data.teamId
            )
            console.log("len", nowMarkersLen);
            markers.splice(nowMarkersLen);
            console.log("demarkers", markers);
            markers.push(...addMarkers);
            console.log("nowmarkers", markers);
        } else { //个人赛
            addMarkers = allUserMarkers.find(item => item.clientId === that.data.options.clientId)
            markers.splice(nowMarkersLen);
            markers.push(addMarkers);
        }
        that.setData({
    ...
    getMaxMinLongitudeLatitude() { //默认附近0.080千米
        ...
        let reachPoints = this.data.reachPoints;
        // 有任务点之后才开始
        let allFinish
    ...
    // 跳转到排行榜页面
    checkRanking(e, isEnd = false) {
        console.log(isEnd);
        let that = this;
        let users = that.data.allUserMarkers;
        // 所有用户
        ...
        if (isEnd) {
            wx.redirectTo({
                url: '/subPackages/pages/ranking/ranking?id=' + that.data.roomId + '&users=' + users_json
            })
        } else {
            wx.navigateTo({
                url: '/subPackages/pages/ranking/ranking?id=' + that.data.roomId + '&users=' + users_json
    ...
    CalFinish() {
        ...
        // 修改缓存endState
        let tempList = wx.getStorageSync('itemInfo');
        let findidx = tempList.findIndex(item => item._id == that.data.roomId)
        tempList[findidx].endState = true;
        wx.setStorageSync('itemInfo', tempList)
        // 修改数据库endState
        activity
            .doc(that.data.roomId)
            .update({
                data: {
                    users: _.push(...users),
                    endState: true,
                ...
                that.logout();
                that.checkRanking(undefined, true);
            })
    },
